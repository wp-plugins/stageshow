...
if (!class_exists('StageShowLibPayPalSimulator')) 
{
class StageShowLibPayPalSimulator
{
	...
	function OutputSaleForm($saleId)
	{
		...
		$formHTML .= $this->PayPalTags($this->myDBaseObj->StageshowOptionsName, true);
		$formHTML .= $this->OutputActionsTable();	
		$formHTML .= "</table>\n";			
		$formHTML .= "<div>\n";			
		
		$formHTML .= $this->PayPalTags($this->myDBaseObj->StageshowOptionsName, false);
    ...
    function AddHiddenTag($tagName, $tagValue, $editable = false, $state = false) 
	{
		...
		$tagValue = $this->myDBaseObj->getDbgOption('PayPalVal_'.$tagName);
		if ($tagValue != '')
		{
			$tagValue = $tagValue.'';
		}
...
//$ourObj = new StageShowLibPayPalSimulator();
