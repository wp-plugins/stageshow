...
if (defined('MJSLIB_TRACK_INCLUDES_FILE'))
{
	include MJSLIB_TRACK_INCLUDES_FILE;
...
if (!class_exists('PayPalAPIClass')) 
{
	class PayPalAPIClass // Define class
	{
		...
		function CheckIsConfigured()
		{
			...
			$settingsPageId = basename(dirname(dirname($this->caller)))."_settings";
		...
		function SetTestMode($testmode, $useLocalIPNServer = false)
		{
			if ($testmode)
			{
				$this->APIEndPoint = 'https://api-3t.sandbox.paypal.com/nvp';
				$this->PayPalURL = $this->GetURL('https://www.sandbox.paypal.com/cgi-bin/webscr');
			}
			else 
			{
				$this->APIEndPoint = 'https://api-3t.paypal.com/nvp';
				$this->PayPalURL = $this->GetURL('https://www.paypal.com/cgi-bin/webscr');
			}
		...
		function SetLoginParams($loginEnv, $username, $password, $signature, $currency = '', $email = '', $useLocalIPNServer = false)		
		{
			...
			$this->SetTestMode($loginEnv == 'sandbox', $useLocalIPNServer);
